...
import com.logex.fragmentation.debug.DebugFragmentRecord;
import com.logex.fragmentation.debug.DebugHierarchyViewContainer;
...
class FragmentationDelegate {
    ...
    FragmentationDelegate(BaseActivity activity) {
     ...
     * @param to          目标Fragment
     */
    void loadRootTransaction(FragmentManager fragmentManager, int containerId, BaseFragment to) {
        BaseFragment fragment = findFragmentByRoot(fragmentManager, to.getClass().getName());
        if (fragment != null && containerId == fragment.getContainerId()) {
            return;
        }
        bindContainerId(containerId, to);
        dispatchStartTransaction(fragmentManager, null, to, 0, BaseFragment.STANDARD, TYPE_ADD);
    }
     ...
     * @param to          目标Fragment
     */
    void replaceLoadRootTransaction(FragmentManager fragmentManager, int containerId, BaseFragment to, boolean addToBack) {
     ...
     * 加载多个根Fragment
     */
    void loadMultipleRootTransaction(FragmentManager fragmentManager, int containerId, int showPosition, BaseFragment... tos) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
        FragmentTransaction ft = fragmentManager.beginTransaction();
        for (int i = 0; i < tos.length; i++) {
            BaseFragment to = tos[i];
     ...
     * @param type        类型
     */
    void dispatchStartTransaction(FragmentManager fragmentManager, BaseFragment from, BaseFragment to, int requestCode, int launchMode, int type) {
        fragmentManager = checkFragmentManager(fragmentManager, from);
        if (fragmentManager == null) return;

        if ((from != null && from.isRemoving())) {
            LogUtil.i(from.getClass().getSimpleName() + " is poped, maybe you want to call startWithPop()!");
            return;
        }
     ...
     */
    void replaceTransaction(BaseFragment from, BaseFragment to, boolean addToBack) {
     ...
     * replace事务, 主要用于子Fragment之间的replace
     */
    void replaceTransaction(FragmentManager fragmentManager, int containerId, BaseFragment to, boolean addToBack) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
     ...
     */
    void showHideFragment(FragmentManager fragmentManager, BaseFragment showFragment, BaseFragment hideFragment) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
    ...
    void start(FragmentManager fragmentManager, BaseFragment from, BaseFragment to, String toFragmentTag, ArrayList<TransactionRecord.SharedElement> sharedElementList) {
        FragmentTransaction ft = fragmentManager.beginTransaction();
    ...
    void startWithPop(final FragmentManager fragmentManager, BaseFragment from, BaseFragment to, String toFragmentTag) {
        fragmentManager.executePendingTransactions();
        if (from.isHidden()) {
            LogUtil.e(from.getClass().getSimpleName() + " is hidden, " + "the transaction of startWithPop() is invalid!");
            return;
        }
    ...
    private void supportCommit(FragmentManager fragmentManager, FragmentTransaction transaction) {
        if (Fragmentation.getDefault().isDebug()) {
            ...
            if (stateSaved) {
                ...
                LogUtil.e("Please beginTransaction in onPostResume() after the Activity returns!");
     ...
     * 获得栈顶SupportFragment
     */
    BaseFragment getTopFragment(FragmentManager fragmentManager) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
    ...
    <T extends BaseFragment> T findStackFragment(Class<T> fragmentClass, String toFragmentTag, FragmentManager fragmentManager) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return null;
        ...
        if (toFragmentTag == null) {
            ...
            for (int i = sizeChildFrgList - 1; i >= 0; i--) {
                ...
                if (brotherFragment instanceof BaseFragment && brotherFragment.getClass().getName().equals(fragmentClass.getName())) {
     ...
     */
    private BaseFragment findFragmentByRoot(FragmentManager fragmentManager, String tag) {
        List<Fragment> fragmentList = fragmentManager.getFragments();
        if (fragmentList != null) {
            for (Fragment fragment : fragmentList) {
                if (fragment instanceof BaseFragment && tag.equals(fragment.getTag())) {
                    return (BaseFragment) fragment;
                }
            }
        }
    ...
    BaseFragment getActiveFragment(BaseFragment parentFragment, FragmentManager fragmentManager) {
        ...
        for (int i = fragmentList.size() - 1; i >= 0; i--) {
            Fragment fragment = fragmentList.get(i);
            if (fragment instanceof BaseFragment) {
                BaseFragment supportFragment = (BaseFragment) fragment;
     ...
     * handle LaunchMode
     */
    private boolean handleLaunchMode(FragmentManager fragmentManager, BaseFragment toFragment, String toFragmentTag, int launchMode) {
        ...
        if (launchMode == BaseFragment.SINGLETOP) {
            // 在栈顶
        ...
        } else if (launchMode == BaseFragment.SINGLETASK) {
    ...
    private void handleNewBundle(BaseFragment toFragment, Fragment stackToFragment) {
        ...
        ((BaseFragment) stackToFragment).onNewBundle(args);
    }
    ...
    private void debouncePop(FragmentManager fm) {
        ...
        if (popF instanceof BaseFragment) {
            BaseFragment supportF = (BaseFragment) popF;
    ...
    void popTo(String fragmentTag, boolean includeSelf, Runnable afterPopTransactionRunnable, FragmentManager fragmentManager) {
        ...
        if (targetFragment == null) {
            LogUtil.e("Pop failure! Can't find FragmentTag:" + fragmentTag + " in the FragmentManager's Stack.");
            return;
        }
     ...
     */
    private void handlePopAnim(Fragment targetFragment, BaseFragment fromFragment, BaseFragment toFragment) {
        ...
        try {
            ...
            if (container != null) {
                ...
                if (preViewGroup != null) {
                    ...
                    restoreView(viewGroup, fromView, toFragment == null ? BUFFER_TIME :
                            Math.max(fromFragment.getEnterAnimDuration(), fromFragment.getPopExitAnimDuration()) + BUFFER_TIME);
                }
            }
    ...
    private FragmentManager checkFragmentManager(FragmentManager fragmentManager, Fragment from) {
        if (fragmentManager == null) {
            if (mPopToTempFragmentManager == null) {
                ...
                LogUtil.e(fromName + "'s FragmentManager is null, " + " Please check if " + fromName + " is destroyed!");
                return null;
            }
