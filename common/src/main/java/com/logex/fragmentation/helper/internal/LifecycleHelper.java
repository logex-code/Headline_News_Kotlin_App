...
public class LifecycleHelper {
    ...
    public void dispatchLifecycle(int lifecycle, BaseFragment fragment, Bundle bundle, boolean visible) {
        switch (lifecycle) {
...
//                break;
    ...
    private void dispatchFragmentSaveInstanceState(BaseFragment fragment, Bundle outState) {
    ...
    private void dispatchFragmentEnterAnimationDispatchEnd(BaseFragment fragment, Bundle savedInstanceState) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentEnterAnimationEnd(fragment, savedInstanceState);
        }
    }

    private void dispatchFragmentLazyInitView(BaseFragment fragment, Bundle savedInstanceState) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentLazyInitView(fragment, savedInstanceState);
        }
    }

    private void dispatchFragmentSupportVisible(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentSupportVisible(fragment);
        }
    }

    private void dispatchFragmentSupportInvisible(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentSupportInvisible(fragment);
        }
    }

    private void dispatchFragmentAttached(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentAttached(fragment);
        }
    }

    private void dispatchFragmentCreated(BaseFragment fragment, Bundle savedInstanceState) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
...
//    }
    ...
    private void dispatchFragmentViewCreated(BaseFragment fragment, Bundle savedInstanceState) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentViewCreated(fragment, savedInstanceState);
        }
    }

    private void dispatchFragmentActivityCreated(BaseFragment fragment, Bundle savedInstanceState) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentActivityCreated(fragment, savedInstanceState);
        }
    }

    private void dispatchFragmentStarted(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentStarted(fragment);
        }
    }

    private void dispatchFragmentResumed(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentResumed(fragment);
        }
    }

    private void dispatchFragmentPaused(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentPaused(fragment);
        }
    }

    private void dispatchFragmentStopped(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentStopped(fragment);
        }
    }

    private void dispatchFragmentDestroyView(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentDestroyView(fragment);
        }
    }

    private void dispatchFragmentDestroyed(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
            mFragmentLifecycleCallbacks.get(i).onFragmentDestroyed(fragment);
        }
    }

    private void dispatchFragmentDetached(BaseFragment fragment) {
        for (int i = 0; i < mFragmentLifecycleCallbacks.size(); i++) {
